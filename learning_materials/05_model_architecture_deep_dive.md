# 第05章：Model.py 架构深度解析 - Transformer 完全指南

> **学习目标**: 深入理解GPT模型的内部结构和工作原理  
> **难度等级**: 🌿🌿🌿 进阶（需要Python和深度学习基础）  
> **预计时间**: 4-6小时（建议分2-3天学习）  
> **前置知识**: 01配置参数、02数据加载、03训练循环、04完整指南

## 🎯 你将学到什么

学完本章，你将能够：
- ✅ 理解Transformer架构的每个组件及其作用
- ✅ 掌握Self-Attention机制的工作原理
- ✅ 理解位置编码、残差连接、LayerNorm的必要性
- ✅ 能够阅读和修改model.py源代码
- ✅ 理解GPT如何生成文本的完整流程
- ✅ 掌握Flash Attention等优化技巧
...
**比喻**：如果说训练是"开车"，那么理解架构就是"了解发动机原理"。
- 🚗 开车：你能用，但不知道为什么
- 🔧 懂原理：你能修、能改、能优化

**学完之后**：
- ✅ 你能看懂任何Transformer模型的代码
- ✅ 你能自己实现和改进模型
- ✅ 你能理解最新的架构创新
- ✅ 你能针对性地优化性能
...
**基础理解（必须掌握）**：
- [ ] 能解释Transformer的整体架构（Embedding → Blocks → LM Head）
- [ ] 理解Self-Attention的Q、K、V是什么
- [ ] 知道为什么需要位置编码
- [ ] 理解残差连接的作用
- [ ] 能解释LayerNorm的必要性
- [ ] 知道MLP在Transformer中的作用

**深入理解（建议掌握）**：
- [ ] 能手写Attention的计算公式
- [ ] 理解Causal Mask的作用和实现
- [ ] 知道Multi-Head Attention为什么有效
- [ ] 理解Flash Attention的优化原理
- [ ] 能解释为什么需要多层Transformer
- [ ] 理解参数量和计算量的关系
...
# 三步走
Q, K, V = 查询、键、值
attention_weights = softmax(Q @ K.T / √d)  # 计算相关性
...
**1. 深入理论** → 学习第06章：Scaling Laws
- 理解模型大小与性能的关系
- 学习如何设计更大的模型

**2. 优化性能** → 学习第07章：架构改进技术
- RoPE、ALiBi等位置编码改进
- Flash Attention、MQA/GQA优化
- RMSNorm、SwiGLU等新技术

**3. 分布式训练** → 学习第08章：分布式训练
- 如何训练更大的模型
- DDP、FSDP、DeepSpeed
...
```bash
# 1. 可视化Attention权重
python visualize_attention.py --checkpoint ckpt.pt

# 2. 实验不同的模型大小
python train.py --n_layer 6 --n_head 6 --n_embd 384

# 3. 对比Flash Attention的速度
python benchmark_attention.py
```

**深入研究**：
1. 阅读《Attention is All You Need》原论文
2. 实现自己的Attention变体
3. 尝试添加新的架构组件
...
```python
# 不除的话
Q @ K.T = 很大的数 → softmax → [0, 0, 1, 0] (太尖锐)

# 除以√d_k后
Q @ K.T / √d_k = 适中的数 → softmax → [0.1, 0.3, 0.4, 0.2] (更平滑)
```

### Q2: Multi-Head Attention为什么有效？
**A**: 类似"多个专家"，每个head关注不同的模式。
```
Head 1: 关注语法关系（主谓宾）
Head 2: 关注语义关系（同义词）
Head 3: 关注长距离依赖
...
### Q3: 位置编码为什么不用学习的？
**A**: 两种都可以！
- **固定位置编码**：泛化性好，可以处理更长的序列
- **学习位置编码**：更灵活，但可能过拟合

GPT-2用的是学习的，BERT用的是固定的。

### Q4: 为什么GPT只用Decoder不用Encoder？
**A**: 因为任务不同。
- **Encoder-Decoder**：翻译（需要看整个输入）
- **Decoder-Only**：生成（只能看过去）

GPT是生成模型，所以只需要Decoder。

### Q5: Flash Attention真的快很多吗？
**A**: 是的！
```
标准Attention: O(N²) 显存
...
```python
# 1. 检查Attention权重
attn_weights = model.transformer.h[0].attn.c_attn.weight
print(attn_weights.abs().mean())  # 应该在0.01-0.1之间

# 2. 检查是否有梯度
print(attn_weights.grad)  # 不应该是None
...
### Q9: 如何实现自定义的Attention？
**A**: 修改CausalSelfAttention类：
```python
class MyCustomAttention(nn.Module):
    def forward(self, x):
        ...
        # 例如：添加相对位置编码
        att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))
        att = att + self.relative_position_bias  # 你的创新！
        att = F.softmax(att, dim=-1)
...
### Q10: 学完这章后能做什么项目？
**A**: 很多！
1. **实现Attention变体**：Sparse Attention、Linear Attention
2. **添加新功能**：相对位置编码、ALiBi
3. **模型压缩**：知识蒸馏、剪枝
4. **可视化工具**：Attention热力图、激活分析
5. **性能优化**：自定义CUDA kernel
